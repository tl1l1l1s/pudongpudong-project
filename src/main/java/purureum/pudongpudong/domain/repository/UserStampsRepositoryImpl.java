...
public class UserStampsRepositoryImpl implements UserStampsRepositoryCustom {
	...
	@Override
	public List<UserStamps> findByUserAndDateRangeWithDetails(Long userId, java.time.LocalDateTime startDate, java.time.LocalDateTime endDate) {
		return queryFactory
				.selectFrom(userStamps)
				.join(userStamps.session, sessions).fetchJoin()
				.join(sessions.park, parks).fetchJoin()
				.join(userStamps.species, species).fetchJoin()
				.where(userStamps.user.id.eq(userId)
						.and(sessions.createdAt.between(startDate, endDate)))
				.orderBy(userStamps.createdAt.desc())
				.fetch();
	}
}